...
`v1.13.0`부터는 **작업하던 파일이 저장되지 않은 상태로 남아 있어도 그대로 시작하면 된다.** 예전에는 먼저 정리하면 제안이 버려지고, 나중에 정리하면 그 작업이 이번 버전 기록에 섞였기 때문에 사용자가 정리 시점을 맞춰야 했다. 이제 Shipping Harness가 분석을 시작하기 전에 현재 작업 트리를 **로컬 커밋 하나**로 먼저 남긴다. 되돌리려면 `git reset --soft HEAD~1` 한 줄이면 되고, 이 문구는 응답 첫 줄에 항상 함께 표시된다. 원격 저장소에 올리거나(push) 태그를 달거나 브랜치를 바꾸는 일은 절대 하지 않는다. `.gitignore`가 무시하는 파일과 `.shipping/` 내부 상태는 어떤 경우에도 커밋되지 않으며, `.env`·`id_rsa`·`credentials.json` 같은 자격증명류 파일이 남아 있으면 **커밋 자체를 하지 않고** 그 파일 이름을 알려주고 멈춘다. 작업 트리를 그대로 두고 싶으면 "커밋하지 말고 시작해"라고 말하면 된다.
