...
| `shipping_status` | Read-only: current Shipping Harness state, blockers, evidence freshness, Git state, and next action. | none | Any state, including uninitialized. | A user status view with state, blockers, freshness, and next action. |
