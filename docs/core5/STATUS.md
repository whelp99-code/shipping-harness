...
>
> H03 PASS (2026-09-22): authenticated Core5 review UI on `http://127.0.0.1:4174/` (Basic `core5`, not `:4173`). systemd user unit `core5-review-ui.service` is active. Unauthenticated GET is 401. `npm run h03` in `ci/core5` — 2 passed / 0 failed. Live `core5 decision` H03 probe passes when `CORE5_UI_BASE_URL` + `CORE5_REVIEW_ACCESS_KEY` are sourced from `~/.config/core5/review.env`.
>
