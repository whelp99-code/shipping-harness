...
3. Shipping의 Proposal ID, Revision, canonical state, 선택 Workspace, 명령과 각 \`cwd\`, baseline, intelligence, coverage만 권위 있는 계획으로 사용한다. 추론한 목표는 추천일 뿐 사용자 목표를 덮어쓰지 않는다.
4. \`NEEDS_INPUT\`, \`DIRTY_BASELINE\`, \`NEEDS_ACCEPTANCE\`를 승인 가능 상태로 표현하지 않는다.
5. \`DIRTY_BASELINE\`이면 정확한 보존 계획을 먼저 보여준다. Shipping은 commit, stash, reset, discard를 실행하지 않는다. 사용자가 별도로 승인한 정확한 host commit 뒤에만 plan hash와 commit SHA로 같은 Proposal을 rescan한다.
6. 선택·질문·기준선 rescan이 필요하면 새 Proposal을 만들지 않고 \`mcp__shipping_harness_shipping_refine\`을 사용한다. 실제 변화가 없는 \`changed: false\`를 새 Revision으로 보고하지 않는다.
7. changed product path가 Acceptance coverage에 연결되지 않으면 \`NEEDS_ACCEPTANCE\`를 유지한다. 모델 설명으로 검증을 보강하지 않는다.
8. package/release 검증은 Shipping이 표시한 isolation·determinism 정책을 약화하지 않는다. 외부상태·데이터상태 명령을 자동 실행하지 않는다.
9. 사용자가 one-screen 범위를 명시적으로 승인하기 전에는 \`mcp__shipping_harness_shipping_approve_scope\`를 호출하지 않는다.
10. 승인 후 잠긴 범위와 예산만 구현하고 \`mcp__shipping_harness_shipping_verify\`를 호출한다. BLOCKER만 수정하고 NEXT는 다음 버전으로 넘긴다.
11. 모델 문장이나 OMP Task 완료를 완료 증거로 보지 않는다. Shipping 상태가 CLOSED일 때만 완료를 보고한다.
12. 사용자 pause/abort가 모든 자동 재개보다 우선한다. 자동 push, merge, deploy, 구매, 고객 연락, 공개 노출은 별도 승인 없이는 수행하지 않는다.
13. CLOSED 버전을 다시 열지 않고 새 요구는 더 높은 버전 계약으로 시작한다.
${OMP_AGENT_BLOCK_END}`;
