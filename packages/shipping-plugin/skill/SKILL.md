...
0. If the project already has a plan (`docs/planning/`, `STATUS.json`, a roadmap), and `docs/shipping-plan.json` does not yet exist, offer to read it and write `docs/shipping-plan.json` (schema `shipping-harness/plan-v1`, see `docs/SHIPPING-PLAN.md`) — never any `command`/`shell`/`args`/`argv`/`env`/`environment` key, only stage `acceptanceRefs` from `shipping-harness plan check --json`. This is a normal file the user reviews and commits; Shipping Harness never writes or executes it.
